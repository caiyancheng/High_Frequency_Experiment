...
用法：
  python sort_moa_csv.py                        # 默认输入/输出 MOA_results.csv
  python sort_moa_csv.py -i raw.csv -o sorted.csv
"""
...
def sort_key(row: dict):
    return (
        ...
        int(row["repeat_index"]),                            # 6. 重复序号
    )


def sort_moa_csv(input_path: str, output_path: str):
...
def main():
    parser = argparse.ArgumentParser(description="Sort MOA_results.csv")
    parser.add_argument("-i", "--input",  default="MOA_results.csv",
                        help="输入 CSV 路径（默认: MOA_results.csv）")
    parser.add_argument("-o", "--output", default=None,
                        help="输出 CSV 路径（默认: 覆盖原文件）")
    args = parser.parse_args()

    output = args.output if args.output else args.input
    sort_moa_csv(args.input, output)
